...
object Rewrite {
  def main(args: Array[String]): Unit = {
    ...
    mirror.sources.foreach(source => {
      logger.elem(source)
      implicit val ctx = rewrite.RewriteCtx(source, config.ScalafixConfig(), mirror)

      val patches = FunctorToZ.rewriteFunctor(source, ctx) ++
          SemigroupToZ.rewriteSemigroup(source, ctx) ++
          EqToZ.rewriteEq(source, ctx)

      val patched = Patch(source, patches)
